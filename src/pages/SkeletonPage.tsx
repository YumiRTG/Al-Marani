      ...
      <main className="max-w-[1320px] mx-auto px-4 sm:px-6 pt-10 pb-24">
        ...
        <section className="mt-9 rounded-[32px] bg-white shadow-xl overflow-hidden">
          ...
          <div className="p-5 grid lg:grid-cols-[230px_1fr] gap-5">
            <aside className="space-y-3">
              ...
              <button onClick={() => { setSide('front'); setMode('numbered'); }} className={`w-full flex gap-2 rounded-xl p-3 font-bold ${mode === 'numbered' ? 'bg-violet-600 text-white' : 'bg-slate-50'}`}><GraduationCap className="w-4 h-4" /> Mit Nummern</button>
        ...
        <section className="mt-11">
          ...
          <div className="mt-6 grid gap-5">
            {groups.map(([title, items]) => <article key={title} className="rounded-3xl bg-white shadow-lg p-6"><h3 className="text-xl font-black">{title}</h3><div className="mt-4 grid md:grid-cols-2 gap-3">{items.map(([de, la, n]) => <div key={de + la} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 flex justify-between gap-4"><div><div className="font-black">{de}</div><div className="text-sm italic text-slate-500">{la}</div></div><span className="text-xs font-black bg-white border border-slate-200 rounded-lg px-2 py-1 h-fit">{n}</span></div>)}</div></article>)}
          </div>
